...
const CommunityReports = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Page Header */}
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-slate-800 dark:text-slate-100">Community Proximity Alerts</h1>
        <p className="text-sm text-slate-500 mt-1">Review crowd-sourced alerts and disease outbreak clusters reported by nearby farmers.</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 p-4 rounded-2xl flex flex-col md:flex-row gap-4 justify-between items-center shadow-sm">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3.5 py-2 border border-slate-200 dark:border-slate-800 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 font-semibold cursor-pointer text-xs"
        >
          <RefreshCw size={13} />
          Reload Outbreaks
        </button>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 rounded-2xl shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-20 px-4">
            ...
            <button
              onClick={fetchReports}
              className="mt-4 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-semibold text-xs cursor-pointer"
            >
              Retry Connection
            </button>
          </div>
          ...
          <div className="text-center py-20 px-4">
            ...
            <p className="text-slate-400 text-xs mt-1">outbreak alerts will appear here when nearby farmers report crop infections.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 text-xs">
                {reports.map((report) => (
                  <tr key={report._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-950/20 transition-colors">
                    <td className="p-4 font-semibold text-emerald-650 dark:text-emerald-400">
                      {report.crop}
                    </td>
                    ...
                    <td className="p-4 text-center">
                      <span className="inline-block text-[11px] font-bold px-2.5 py-0.5 rounded bg-red-50 text-red-650 dark:bg-red-950/30 dark:text-red-400">
        ...
        {!loading && !error && totalPages > 1 && (
          <div className="flex items-center justify-between p-4 border-t border-slate-100 dark:border-slate-800/80 text-xs text-slate-500 shadow-inner">
            <button
              disabled={page === 1}
