...
import { Sun, ShieldCheck, Droplets, TrendingUp, TrendingDown, AlertTriangle, Sprout } from 'lucide-react';
...
const HeaderStats = ({
  ...
  const getGrowthProgress = (stage) => {
    switch (stage.toLowerCase()) {
      case 'germination': return 15;
      case 'seedling': return 30;
      case 'vegetative': return 50;
      case 'flowering': return 75;
      case 'milking': return 90;
      case 'maturity': return 100;
      default: return 50;
    }
  };
      ...
      <div className="flex gap-4 overflow-x-auto pb-3 pt-1 scrollbar-none snap-x md:grid md:grid-cols-3 xl:grid-cols-6 md:pb-0 md:overflow-x-visible">
        ...
        <div className="min-w-[260px] sm:min-w-0 snap-center bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-5 shadow-xs hover:shadow-md dark:hover:shadow-slate-950 transition-all duration-300 flex flex-col justify-between h-36 group">
          ...
          <div className="flex items-center justify-between mt-3">
            ...
            <svg className="w-6 h-6 -rotate-95" viewBox="0 0 36 36">
              <circle className="text-slate-100 dark:text-slate-800" strokeWidth="3" stroke="currentColor" fill="none" cx="18" cy="18" r="15" />
              <circle className="text-emerald-500" strokeWidth="3" strokeDasharray="92 100" strokeLinecap="round" stroke="currentColor" fill="none" cx="18" cy="18" r="15" />
            </svg>
          </div>
        </div>
        ...
        <div className="min-w-[260px] sm:min-w-0 snap-center bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-5 shadow-xs hover:shadow-md dark:hover:shadow-slate-950 transition-all duration-300 flex flex-col justify-between h-36 group">
          <div className="flex justify-between items-start">
            ...
            <div className={`p-2 rounded-2xl group-hover:scale-110 transition-transform ${isSoilDry ? 'bg-amber-100 dark:bg-amber-950/30 text-amber-600 dark:text-amber-400' : 'bg-blue-100 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400'}`}>
              <Droplets size={18} />
            </div>
          </div>

          <div className="flex items-center justify-between mt-3">
            <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${isSoilDry ? 'text-amber-700 bg-amber-50 dark:bg-amber-950/20 dark:text-amber-400' : 'text-blue-700 bg-blue-50 dark:bg-blue-950/20 dark:text-blue-400'}`}>
        ...
        <div className="min-w-[260px] sm:min-w-0 snap-center bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-5 shadow-xs hover:shadow-md dark:hover:shadow-slate-950 transition-all duration-300 flex flex-col justify-between h-36 group">
          <div className="flex justify-between items-start">
            ...
            <div className={`p-2 rounded-2xl group-hover:scale-110 transition-transform ${isRising ? 'bg-emerald-100 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400' : 'bg-rose-100 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400'}`}>
              {isRising ? <TrendingUp size={18} /> : <TrendingDown size={18} />}
            </div>
          </div>

          <div className="flex items-center justify-between mt-3 gap-2">
            <span className={`text-[11px] font-bold ${isRising ? 'text-emerald-700 bg-emerald-50 dark:bg-emerald-950/20 dark:text-emerald-450' : 'text-rose-700 bg-rose-50 dark:bg-rose-950/20 dark:text-rose-450'} px-2 py-0.5 rounded-full whitespace-nowrap`}>
              {isRising ? `+${changePercent}%` : `-${changePercent}%`} Upward
            </span>
            {/* Sparkline mini chart */}
            <svg className="w-16 h-8 text-emerald-500 dark:text-emerald-400 overflow-visible" viewBox="0 0 100 40">
              <path
                d={isRising 
