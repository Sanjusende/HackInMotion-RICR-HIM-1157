...
const CurrentWeather = ({ weather }) => {
  ...
  const uvIndex = condition.toLowerCase().includes('rain') || condition.toLowerCase().includes('cloud') ? 4 : 7;
  const uvStatus = uvIndex >= 6 ? 'Moderate-High' : 'Low-Moderate';
    ...
    <div className="space-y-4">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-3">
            ...
            <div className="flex items-baseline gap-3">
              <span className="text-5xl sm:text-6xl font-black tracking-tight drop-shadow-xs">{temp}°C</span>
              <div>
            ...
            <div className="flex items-center gap-3 text-xs font-semibold text-emerald-200/90 pt-1 border-t border-white/10">
              ...
              <span>•</span>
              <span className="text-emerald-300 font-bold">{weather?.source === 'open-meteo' ? 'OpenMeteo Live' : 'Cached Data'}</span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="bg-white p-3.5 rounded-2xl border border-slate-200/80 shadow-xs hover:border-emerald-300 transition-colors flex items-center gap-3">
          ...
          <div>
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Rain Chance</span>
        ...
        <div className="bg-white p-3.5 rounded-2xl border border-slate-200/80 shadow-xs hover:border-emerald-300 transition-colors flex items-center gap-3">
          ...
          <div>
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Wind Speed</span>
            <span className="text-lg font-black text-slate-900">{windSpeed} <span className="text-xs font-semibold text-slate-500">km/h</span></span>
          </div>
        </div>
        ...
        <div className="bg-white p-3.5 rounded-2xl border border-slate-200/80 shadow-xs hover:border-emerald-300 transition-colors flex items-center gap-3">
          ...
          <div>
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">UV Index</span>
            <span className="text-lg font-black text-slate-900">{uvIndex} <span className="text-[11px] font-extrabold text-amber-600">({uvStatus})</span></span>
          </div>
        </div>
      </div>
    </div>
