...
const MarketIntelligence = () => {
  ...
  const fetchMarket = async () => {
    try {
      setError(null);
      const res = await getMarketCurrent(selectedCrop);
      if (res && res.success) {
  ...
  const bestMarketItem = (nearbyMarkets.length > 0 ? nearbyMarkets : [
  ...
  ]).reduce((max, item) => (item.price > max.price ? item : max), { price: 0, market: primaryMarket });
    ...
    <div className="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6 text-slate-900 selection:bg-emerald-600 selection:text-white">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2.5">
            <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-900">Market Analysis</h1>
          ...
          <p className="text-xs text-slate-500 font-medium mt-0.5">AI-powered crop price and market intelligence.</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <div className="bg-white p-3.5 rounded-xl border border-slate-200/80 shadow-2xs hover:border-emerald-200 transition space-y-1">
          ...
          <p className="text-xl font-black text-slate-900">₹{currentPrice} <span className="text-xs font-bold text-slate-500">/q</span></p>
        ...
        <div className="bg-white p-3.5 rounded-xl border border-slate-200/80 shadow-2xs hover:border-emerald-200 transition space-y-1">
          ...
          <p className="text-base font-extrabold text-slate-900 truncate">{bestMarketItem.market}</p>
          <p className="text-[10px] font-semibold text-emerald-700">₹{bestMarketItem.price} / Quintal</p>
        </div>

        <div className="bg-white p-3.5 rounded-xl border border-slate-200/80 shadow-2xs hover:border-emerald-200 transition space-y-1">
          <div className="flex items-center justify-between text-slate-500">
            <span className="text-[11px] font-semibold">Market Trend</span>
            {trend === 'Rising' ? <TrendingUp className="w-4 h-4 text-emerald-600" /> : <Minus className="w-4 h-4 text-slate-500" />}
          </div>
          ...
          {['7d', '30d', '90d'].map((p) => (
            <button
              ...
              className={`px-3 py-1 rounded-md uppercase transition cursor-pointer ${
                period === p ? 'bg-white text-slate-900 shadow-2xs font-extrabold' : 'text-slate-600 hover:text-slate-900'
              }`}
            >
              {p}
            </button>
      ...
      <div className="bg-white p-5 rounded-xl border border-slate-200/80 shadow-2xs space-y-3 hover:border-emerald-200 transition">
        <div className="flex items-center justify-between border-b border-slate-100 pb-2">
          <h2 className="text-xs font-bold text-slate-900 uppercase tracking-wider">Market Price Overview — {selectedCrop}</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200/80 shadow-2xs space-y-3 hover:border-emerald-200 transition">
          <div className="flex items-center justify-between border-b border-slate-100 pb-2">
            <h2 className="text-xs font-bold text-slate-900 uppercase tracking-wider">Price Trend ({period})</h2>
            ...
            {historySeries.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={historySeries}>
                  ...
                  />
                  <Line type="monotone" dataKey="price" stroke="#059669" strokeWidth={2.5} dot={{ r: 3, fill: '#059669' }} />
                </LineChart>
              </ResponsiveContainer>
            ) : (
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200/80 shadow-2xs space-y-3 hover:border-emerald-200 transition">
          <div className="flex items-center justify-between border-b border-slate-100 pb-2">
            <h2 className="text-xs font-bold text-slate-900 uppercase tracking-wider">Market Price Comparison</h2>
            <span className="text-[11px] font-semibold text-slate-400">Nearby Mandis</span>
          </div>

          <div className="h-52 w-full pt-2">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={nearbyMarkets.length > 0 ? nearbyMarkets : [
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200/80 shadow-2xs space-y-3 hover:border-emerald-200 transition flex flex-col justify-between">
          <div className="flex items-center justify-between border-b border-slate-100 pb-2">
            <h2 className="text-xs font-bold text-slate-900 uppercase tracking-wider">Best Market</h2>
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs font-extrabold text-emerald-700">₹{bestMarketItem.price} / Quintal</p>
          </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200/80 shadow-2xs space-y-3 hover:border-emerald-200 transition flex flex-col justify-between">
          <div className="flex items-center justify-between border-b border-slate-100 pb-2">
            <h2 className="text-xs font-bold text-slate-900 uppercase tracking-wider">Market Trend</h2>
            <span className="text-[11px] font-extrabold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">
              🟢 {trend}
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200/80 shadow-2xs space-y-3 hover:border-emerald-200 transition flex flex-col justify-between">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div>
              <span className="text-[10px] text-slate-400 font-bold block uppercase">Current Price</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {/* PRICE ALERT FORM */}
        <form onSubmit={handleSetAlert} className="bg-white p-5 rounded-xl border border-slate-200/80 shadow-2xs space-y-3 hover:border-emerald-200 transition flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-2 border-b border-slate-100 pb-2">
              <Bell className="w-4 h-4 text-emerald-600" />
              <h2 className="text-xs font-bold text-slate-900 uppercase tracking-wider">Price Alert</h2>
            </div>

            <div className="space-y-2 pt-2 text-xs">
              <label className="text-[11px] font-bold text-slate-600 block">Target Price (₹/Quintal)</label>
              <input
                type="number"
        ...
        <div className="md:col-span-2 bg-white p-5 rounded-xl border border-slate-200/80 shadow-2xs space-y-3 hover:border-emerald-200 transition">
          <h2 className="text-xs font-bold text-slate-900 uppercase tracking-wider">Market Price Summary Table</h2>
