...
async function getAuthToken() {
  ...
  await axios.post(`${BASE_URL}/farms`, {
    name: "Test Farm",
    currentCrop: "Tomato",
    soilType: "Black Soil",
...
async function generateTestImages() {
  // 1. scan_1.jpg: A green healthy leaf (primarily green pixels)
  const img1 = new Jimp({ width: 100, height: 100, color: 0x228B22ff }); // Forest Green
  const buf1 = await img1.getBuffer("image/jpeg");
  fs.writeFileSync("scan_1.jpg", buf1);

  // 2. scan_2.jpg: A tomato leaf showing brown/yellow blight lesions
  const img2 = new Jimp({ width: 100, height: 100, color: 0x228B22ff });
  // Add some yellow-brown blight pixels (e.g. 35% of the leaf area)
  for (let y = 0; y < 100; y++) {
    for (let x = 0; x < 35; x++) {
      img2.setPixelColor(0x8B5A2Bff, x, y); // Brownish yellow
    }
  }
  ...
  const img3 = new Jimp({ width: 100, height: 100, color: 0x228B22ff });
...
async function uploadImage(token, filepath, description = '') {
  ...
  if (fs.existsSync(filepath)) {
    ...
    try {
      ...
      return { status: err.response?.status, error: err.response?.data?.error, message: err.response?.data?.message };
    }
  }
}
...
async function run() {
  ...
  try {
    ...
    console.log(`  Result: success=${res1.data?.success}, crop=${res1.data?.crop}, health=${res1.data?.health}, disease=${res1.data?.disease}, confidence=${res1.data?.confidence}`);
    ...
    const res2 = await uploadImage(token, 'scan_2.jpg', 'tomato plant observation with leaf blight');
    console.log(`  Status: ${res2.status}`);
    console.log(`  Result: success=${res2.data?.success}, crop=${res2.data?.crop}, health=${res2.data?.health}, disease=${res2.data?.disease}, confidence=${res2.data?.confidence}`);
    ...
    if (res2.status === 200 && res2.data?.health === 'Diseased' && res2.data?.disease.includes('Blight')) {
      console.log("  ✅ MATCHED expected success outcome.");
    } else {
      console.error("  ❌ FAILED expected success outcome.");
    }
    ...
    console.log(`  Result: success=${res3.data?.success}, crop=${res3.data?.crop}, health=${res3.data?.health}, disease=${res3.data?.disease}, confidence=${res3.data?.confidence}`);
    ...
    if (res3.status === 200 && res3.data?.crop === 'Wheat' && res3.data?.disease === 'Yellow Rust') {
      console.log("  ✅ MATCHED expected success outcome.");
    } else {
      console.error("  ❌ FAILED expected success outcome.");
    }
    ...
    const res4 = await uploadImage(token, 'scan_4.jpg', 'selfie of a farmer standing in front of crop');
    ...
    try {
      const voiceRes = await axios.post(`${BASE_URL}/voice/query`, {
      ...
      console.log(`  Result: success=${voiceRes.data?.success}, intent=${voiceRes.data?.intent}, response=${voiceRes.data?.response}`);
